...
export function ObitosPorEstadoCivilChart({ data }) {
  ...
  const dadosComNumeros = dadosValidos.map(d => ({
    ...
    estado_civil_desc: String(d.estado_civil_desc || '').trim()
  })).filter(d => d.estado_civil_desc !== '' && d.total_obitos > 0)
  ...
  try {
    return (
      <div 
        className="obitos-estado-civil-chart-container"
        style={{ 
          width: '100%', 
          maxWidth: '100%',
          minWidth: 0,
      ...
      >
        <style>{`
          .obitos-estado-civil-chart-container {
